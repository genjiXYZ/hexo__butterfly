...
{% note info %}
为什么我要捣鼓下 MongoDB呢 ,就因为他是和js很像的数据库.
...
## [社区安装包直接安装](https://www.mongodb.com/try/download/community)

别忘了`添加环境`和设置 `数据目录`,`日志目录`
...
mongodb+srv://<username>:<password>@<ip/domain><:port>/<dbname>?<options>
...
在我浅显的理解就是先ssh连接 然后在登录本地数据库

所以有两种验证方式 password 和 私钥
...
#### authentication 连接服务器

先要在你的数据库创建 username ,要不怎么验证?
